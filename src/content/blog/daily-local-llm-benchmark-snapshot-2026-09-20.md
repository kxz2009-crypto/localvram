...
description: "Daily 3090 recommendation for ministral-3:14b: moderate performer at 77.0 tok/s, RTX 3090 benchmark data, use-case fit, and local-vs-cloud decision guide."
...
`ministral-3:14b` is a **moderate-speed** general-purpose model on a 24GB RTX 3090 (77.0 tok/s). It is worth testing locally for batch or offline workloads. For real-time interactive use, measure end-to-end latency with your typical prompt length before committing.

`ministral-3:14b` fits comfortably in 24GB at standard quantizations. Monitor VRAM usage if you push context beyond 8K tokens. It ranks **#6 of 18** in throughput among currently measured models on this RTX 3090. The next faster model is `qwen2.5:14b` (84.0 tok/s, 9% faster). The next slower model is `deepseek-r1:14b` (72.9 tok/s, 6% slower).
...
- `ministral-3:14b` vs `qwen3:8b` — same size tier, 77 vs 118 tok/s.
