...
description: "Daily 3090 recommendation for qwen3:8b: fast performer at 120.9 tok/s, RTX 3090 benchmark data, use-case fit, and local-vs-cloud decision guide."
...
`qwen3:8b` is a **fast** general-purpose model on a 24GB RTX 3090 (120.9 tok/s). If it fits your VRAM with headroom for your target context length, it is a strong candidate for daily local use. Download it and validate on your own prompts — the numbers suggest it will handle interactive workloads comfortably.

`qwen3:8b` fits comfortably in 24GB at standard quantizations. Monitor VRAM usage if you push context beyond 8K tokens. It ranks **#3 of 18** in throughput among currently measured models on this RTX 3090. The next faster model is `qwen3-coder:30b` (140.1 tok/s, 16% faster). The next slower model is `qwen2.5:14b` (84.0 tok/s, 44% slower).
...
- `qwen3:8b` vs `qwen2.5:14b` — same size tier, 121 vs 84 tok/s.
