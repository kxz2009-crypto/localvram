...
description: "Daily 3090 recommendation for qwen3:8b: heavy performer at 12.5 tok/s, RTX 3090 benchmark data, use-case fit, and local-vs-cloud decision guide."
...
`qwen3:8b` is a **heavy** model on 24GB VRAM (12.5 tok/s). It is best suited for offline batch processing, proof-of-concept validation, or cloud fallback scenarios. Reduce context or step down quantization before attempting interactive use.

`qwen3:8b` fits comfortably in 24GB at standard quantizations. Monitor VRAM usage if you push context beyond 8K tokens. It ranks **#6 of 18** in throughput among currently measured models on this RTX 3090. The next faster model is `qwen3-coder:30b` (24.9 tok/s, 99% faster). The next slower model is `glm-4.7-flash:bf16` (11.2 tok/s, 11% slower).
...
1. **Cloud may win**: at 12.5 tok/s on 24GB, qwen3:8b may be more cost-effective on RunPod or Vast.
...
- `qwen3:8b` vs `qwen2.5:14b` — same size tier, 13 vs 84 tok/s.
