<!--
auto-translated from src/content/blog/daily-local-llm-benchmark-snapshot-2026-03-26.md
target-locale: zh
...
1.  如果您的目标模型在 VRAM 中有足够的余量，优先选择本地部署，以获得可预测的延迟和更低的长期成本。
2.  如果 p95 延迟或吞吐量未能达到生产目标，将本地部署作为基线，仅在高峰期突发至云端。
3.  如果故障率上升（OOM/重试激增），在横向扩展之前，降低量化级别或减少并发负载。

## 操作清单

-   在代表性提示长度下验证 tokens/s 和延迟。
-   按模型和量化级别跟踪 OOM 和重试计数。
-   每周重新计算盈亏平衡点，针对本地硬件与云租赁的对比。
