<!--
auto-translated from src/content/blog/daily-local-llm-benchmark-snapshot-2026-09-01.md
target-locale: zh
...
`ministral-3:14b` 是一个 **中等速度** 的通用模型，在 24GB RTX 3090 上运行速度为 (78.7 tok/s)。对于批处理或离线工作负载，值得在本地进行测试。对于实时交互使用，在投入使用前，请使用您典型的提示长度测量端到端延迟。

`ministral-3:14b` 在标准量化下可轻松适应 24GB VRAM。如果将上下文推至 8K tokens 以上，请监控 VRAM 使用情况。在目前测量的 RTX 3090 模型中，它的吞吐量排名 **18 个模型中的第 5 位**。下一个更快的模型是 `qwen2.5:14b` (84.0 tok/s，快 7%)。下一个更慢的模型是 `deepseek-r1:14b` (73.4 tok/s，慢 7%)。
...
- RTX 3090 用户，正在决定今晚是否下载 `ministral-3:14b` 进行本地实验。
- 在确定工作流程之前，将本地推理速度与云租赁（RunPod, Vast）进行比较的用户。
- 任何正在构建本地 LLM 工具箱，并希望获得此模型经过验证的基准的用户。
...
- `ministral-3:14b` 与 `qwen3:8b` — 相同尺寸级别，79 tok/s 对比 119 tok/s。
