<!--
auto-translated from src/content/blog/daily-local-llm-benchmark-snapshot-2026-09-09.md
target-locale: zh
...
`qwen3.5:35b` 是一个在 24GB VRAM 上运行的**重量级**模型（2.8 tok/s）。它最适合离线批处理、概念验证或云回退场景。在尝试交互式使用之前，请减少上下文或降低量化级别。

`qwen3.5:35b` 在较高量化级别下接近 24GB 边界。如果您在 RTX 3090 上需要上下文余量，请考虑 Q4 或 Q5。在当前于此 RTX 3090 上测量的模型中，它的吞吐量排名**18 个模型中的第 17 位**。下一个更快的模型是 `qwen3.5:122b`（4.9 tok/s，快 78%）。下一个更慢的模型是 `llama3.3:70b`（1.5 tok/s，慢 89%）。
...
1.  **云端可能更优**：在 24GB 上以 2.8 tok/s 运行，`qwen3.5:35b` 在 RunPod 或 Vast 上可能更具成本效益。
...
- `qwen3.5:35b` 与 `gpt-oss:20b` — 相同尺寸级别，3 tok/s 对比 156 tok/s。
- `qwen3.5:35b` 的本地功耗成本与相同工作负载下 A100 租赁成本的比较。
