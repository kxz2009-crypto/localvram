...
The ROI model selector now uses the same recent-model registry as the homepage, with Qwen3.8 27B selected by default and a custom exact-tag option. Homepage cards deep-link using `?model=<tag>`. The result identifies the selected model; runtime-mode assumptions remain user inputs, not inferred model benchmarks. Optional generated-token mode converts monthly volume and user-measured local/cloud generation speeds to runtime and relative duration. It excludes prompt processing, queuing and startup. Changing the selected model or custom tag clears both speed measurements to avoid reusing evidence across models.
